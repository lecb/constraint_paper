...
# INPUTS
# ----------------------------
UNIVERSE_FILE <- file.path("gnomad_lof_discordance_out", "loeuf_lt_0.2_precision_filtered.csv")
...
GNOMAD_LOF_BGZ <- file.path(getwd(), "gnomad.v2.1.1.all_lofs.txt.bgz")
...
.request_json <- function(
  ...
  for (attempt in seq_len(max_tries)) {
    ...
    if (as.numeric(difftime(Sys.time(), t0, units = "secs")) > max_elapsed) {
      ...
      return(structure(list(), class = "ensembl_null"))
    }
...
get_translation_length_cached <- function(ensp_id, cache_env) {
  ...
  if (is.na(ensp) || ensp == "") return(NA_real_)
  if (exists(ensp, envir = cache_env, inherits = FALSE)) return(get(ensp, envir = cache_env))
  NA_real_
}
...
recover_vep_checkpoint <- function(path) {
  if (!file.exists(path)) return(list(done = logical(0), rel_tbl = tibble::tibble(protein_id=character(0), aa_pos=numeric(0))))
  ck <- readRDS(path)
  if (!is.list(ck) || is.null(ck$done) || is.null(ck$rel_tbl)) {
    return(list(done = logical(0), rel_tbl = tibble::tibble(protein_id=character(0), aa_pos=numeric(0))))
  }
...
to_vep_variant <- function(chrom, pos, ref, alt, id = ".") {
  ...
  if (!is.finite(pos) || pos <= 0) return(NA_character_)
  if (!nzchar(chrom) || !nzchar(ref) || !nzchar(alt)) return(NA_character_)
