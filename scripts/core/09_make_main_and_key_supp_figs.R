message(sprintf("[09_make_main_and_key_supp_figs] start: %s", Sys.time()))
#!/usr/bin/env Rscript
...
message(sprintf("[09_make_main_and_key_supp_figs] end: %s", Sys.time()))

# ---- end-of-run summary (added) ----
for (d in c("figures", "tables", file.path("manuscript_figures","main"), file.path("manuscript_figures","supp"))) {
  if (dir.exists(d)) {
    fs <- list.files(d, full.names = TRUE)
    fs <- fs[file.info(fs)$isdir == FALSE]
    fs <- fs[order(file.info(fs)$mtime, decreasing = TRUE)]
    message("[09_make_main_and_key_supp_figs] latest in ", d, ":")
    if (length(fs) == 0) {
      message("  (no files)")
    } else {
      for (f in head(fs, 8)) message("  ", f)
    }
  }
}
